...
- Excel файл со сводными данными, содержащий два листа:
  ...
  - **Лист "Расчеты"**: Те же базовые колонки (Табельный, ТБ, ГОСБ, ФИО), расчетные колонки по месяцам с различными типами расчетов (как есть, прирост по 2 месяцам, прирост по трем периодам), колонки с рангами для каждой расчетной колонки, итоговые ранги R_FIN для каждого месяца (M-1 до M-12), и колонка "Лучший месяц" с номером месяца (или месяцев через запятую) с наименьшим итоговым рангом
...
- Колонка "Лучший месяц": текстовый формат (например, `M-3` или `M-3, M-5`)
...
- `prepare_summary_data() -> pd.DataFrame` - Подготавливает сводные данные для листа "Данные" (оптимизировано: предварительная индексация и `groupby()` вместо фильтрации в циклах)
- `prepare_calculated_data(summary_df: pd.DataFrame) -> pd.DataFrame` - Подготавливает расчетные данные для листа "Расчеты" с расчетами, рангами и итоговыми рангами R_FIN
- `_calculate_ranks(calculated_df: pd.DataFrame, all_files_sorted: List[Tuple], config_manager) -> pd.DataFrame` - Рассчитывает ранги для каждой колонки с данными
- `_calculate_final_ranks(calculated_df: pd.DataFrame, config_manager) -> pd.DataFrame` - Рассчитывает итоговые ранги R_FIN для каждого месяца и определяет лучший месяц для каждого КМ (получает веса из соответствующих разделов конфигурации)
...
- Для каждого КМ определяется лучший месяц (месяц с наименьшим значением R_FIN)
- Если несколько месяцев имеют одинаковое минимальное значение R_FIN, в колонке "Лучший месяц" выводится список месяцев через запятую (например, "M-3, M-5")
...
- Итоговый файл содержит два листа: "Данные" (суммы по месяцам) и "Расчеты" (расчетные значения согласно типу расчета + ранги + итоговые ранги R_FIN + лучший месяц)
...
- Колонка "Лучший месяц" отображается в текстовом формате (например, "M-3" или "M-3, M-5")
