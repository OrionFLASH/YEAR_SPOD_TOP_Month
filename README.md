...
2. **Сбор уникальных табельных номеров:**
   ...
   - **Алгоритм поиска ТБ:**
     ...
     - В логах выводится информация о всех найденных вариантах ТБ с суммами и выбранном варианте (только если `ENABLE_DETAILED_TB_VARIANTS_LOGGING = True`)
     - Периодический вывод прогресса в консоль (раз в 15 секунд) работает всегда, независимо от `ENABLE_DETAILED_TB_VARIANTS_LOGGING`
...
# Если False, эта информация не логируется (но периодический вывод прогресса в консоль продолжает работать раз в 15 секунд)
...
**Исправлены критические ошибки нормализации:**
- ✅ Исправлена ошибка `ValueError: The truth value of an array with more than one element is ambiguous` в методе `_is_debug_tab_number`
- ✅ Исправлены все места, где передавался список `DEBUG_TAB_NUMBER` вместо конкретного табельного номера в `debug_tab`
- ✅ Исправлена логика присваивания нормализованных колонок: добавлена проверка совпадения индексов и использование пересечения индексов
- ✅ Исправлена логика создания маски для отладки: используется метод `_create_debug_tab_mask` вместо `str(DEBUG_TAB_NUMBER)`
- ✅ Добавлено расширенное логирование для диагностики проблем с нормализацией:
  * Логирование поиска колонок для нормализации
  * Логирование создания каждой нормализованной колонки
  * Логирование возвращаемых значений из `_normalize_group`
  * Проверка нормализованных колонок до и после `reset_index`
- ✅ Улучшена обработка массивов и Series в `_is_debug_tab_number`: добавлена защита от ошибок при проверке `pd.isna()` для массивов

**Результат:**
- Нормализация теперь работает корректно без ошибок ValueError
- Нормализованные колонки корректно добавляются в `normalized_df`
- Расширенное логирование помогает диагностировать проблемы с нормализацией

### Версия 1.13.0
**Дата:** 2025-12-15

**Добавлен параметр управления детальным логированием вариантов ТБ:**
- ✅ Добавлен параметр `ENABLE_DETAILED_TB_VARIANTS_LOGGING` (bool, по умолчанию False): триггер для включения/выключения детального логирования вариантов ТБ для каждого табельного номера
  * Если True - в лог записывается подробная информация: "для табельного: XXX найдено N вариантов ТБ: ..."
  * Если False - эта информация не логируется (по умолчанию выключено)
  * Периодический вывод прогресса в консоль (раз в 15 секунд) продолжает работать всегда, независимо от этого параметра
- ✅ Оптимизация логирования: детальное логирование вариантов ТБ теперь выполняется только при необходимости, что уменьшает объем логов и ускоряет работу
