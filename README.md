...
# raw_df все равно готовится (нужен для подсчета уникальных ИНН)
...
- `prepare_raw_data() -> pd.DataFrame` - Подготавливает сырые данные для листа "RAW" (параллельная обработка всех файлов). Также собирает данные для debug_tracker
...
- **RAW листы:** Параметр `ENABLE_RAW_SHEETS` позволяет отключить создание RAW листов для ускорения работы и уменьшения размера файла. raw_df все равно готовится (нужен для подсчета уникальных ИНН)
