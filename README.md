...
- Excel файл со сводными данными, содержащий два листа:
  ...
  - **Лист "Расчеты"**: Те же базовые колонки (Табельный, ТБ, ГОСБ, ФИО), расчетные колонки по месяцам с различными типами расчетов (как есть, прирост по 2 месяцам, прирост по трем периодам), и колонки с рангами для каждой расчетной колонки
...
- `prepare_calculated_data(summary_df: pd.DataFrame) -> pd.DataFrame` - Подготавливает расчетные данные для листа "Расчеты" с расчетами и рангами
...
- Итоговый файл содержит два листа: "Данные" (суммы по месяцам) и "Расчеты" (расчетные значения согласно типу расчета + ранги)
